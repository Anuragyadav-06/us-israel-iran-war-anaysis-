# us-israel-iran-war-anaysis-
# Investor Behavior During the 2026 US-Israel-Iran War 📊

## What This Project Analyzes
How Gold, Oil, S&P 500, Defense Stocks and Fear Index 
moved across 3 phases of the 2026 Iran War

## 5 Key Findings
- Gold peaked BEFORE war started — Buy Rumor Sell News
- Oil surged +24.5% — the only real war trade
- Fear (VIX) correlated -0.71 with stocks, not gold
- S&P 500 most volatile DURING ceasefire, not war
- Defense stocks barely moved — war was too short

## Tools Used
Python | Pandas | yfinance | Matplotlib | Seaborn | Power BI

## Data Range
January 1, 2026 — May 30, 2026

## Assets Analyzed
Gold (GC=F) | WTI Oil (CL=F) | S&P 500 (^GSPC) | 
Fear Index (^VIX) | Lockheed Martin (LMT) | Raytheon (RTX)

### Tools
Python | Pandas | yfinance | Matplotlib | Seaborn
##finding 1
<img width="1025" height="443" alt="image" src="https://github.com/user-attachments/assets/57da93e8-d07d-4cd4-99c6-9082fb97c998" />
##finding 2
<img width="1023" height="447" alt="image" src="https://github.com/user-attachments/assets/6412a9cf-54fe-4743-97ec-2a67a91249fb" />
##finding 3
<img width="1041" height="813" alt="image" src="https://github.com/user-attachments/assets/3e39dc78-5bcd-4d1f-95e4-d7bbb1059b9c" />
##fiding 5
<img width="656" height="223" alt="image" src="https://github.com/user-attachments/assets/46facb8b-469c-4997-a239-ef48eb0cd796" />
##finding 5
<img width="1052" height="536" alt="image" src="https://github.com/user-attachments/assets/6c8dfd47-bba2-4f9b-9556-eb971703a74c" />
##conclusion
<img width="1060" height="521" alt="image" src="https://github.com/user-attachments/assets/d79b157b-7298-4460-b4a3-60b6535944f8" />
